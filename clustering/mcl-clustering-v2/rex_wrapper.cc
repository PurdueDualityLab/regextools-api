...
TempRegexFile::TempRegexFile(const std::string &regex) {
    // Create a new temp file for this regex
    this->path = std::filesystem::temp_directory_path() / ("regex_" + std::to_string(random()));
    std::ofstream tmp_file(this->path);
    tmp_file << regex << std::endl;
}

TempRegexFile::~TempRegexFile() {
    // Delete this file
